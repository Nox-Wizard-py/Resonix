...
internal class HighlightNode(
    ...
    override fun ContentDrawScope.draw() {
        val highlight = highlight()
        if (highlight == null || highlight.width.value <= 0f) return drawContent()
    ...
    override fun onDetach() {
        requireGraphicsContext().let { ctx ->
            highlightLayer?.let { ctx.releaseGraphicsLayer(it) }
            highlightLayer = null
        }
        clipPath = null
    ...
    private fun DrawScope.configurePaint(highlight: Highlight) {
        ...
        if (isRuntimeShaderSupported()) {
            val shader = with(highlight.style) { createShader(shapeProvider.shape, runtimeShaderCache) }
