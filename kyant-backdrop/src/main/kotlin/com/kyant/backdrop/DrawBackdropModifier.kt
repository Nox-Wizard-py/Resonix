...
fun Modifier.drawBackdrop(
    ...
    return this
        ...
        .then(if (shadow != null) ShadowElement(shapeProvider, shadow) else Modifier)
        .then(if (highlight != null) HighlightElement(shapeProvider, highlight) else Modifier)
        .then(
            DrawBackdropElement(
                backdrop = backdrop,
                shapeProvider = shapeProvider,
...
private class DrawBackdropElement(
    ...
    override fun create(): DrawBackdropNode = DrawBackdropNode(
        backdrop, shapeProvider, effects, layerBlock, exportedBackdrop,
        onDrawBehind, onDrawBackdrop, onDrawSurface, onDrawFront
    )
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return backdrop == other.backdrop && shapeProvider == other.shapeProvider &&
                effects == other.effects && layerBlock == other.layerBlock &&
                exportedBackdrop == other.exportedBackdrop && onDrawBehind == other.onDrawBehind &&
                onDrawBackdrop == other.onDrawBackdrop && onDrawSurface == other.onDrawSurface &&
...
private class DrawBackdropNode(
    ...
    private val drawBackdropLayer: DrawScope.() -> Unit = {
        ...
        if (layer != null) {
            ...
            layer.topLeft = if (padding != 0f) IntOffset(-padding.toInt(), -padding.toInt()) else IntOffset.Zero
            drawLayer(layer)
        }
    }
    ...
    override fun onGloballyPositioned(coordinates: LayoutCoordinates) {
        if (coordinates.isAttached) {
            layoutCoordinates = if (backdrop.isCoordinatesDependent) coordinates else {
                if (layoutCoordinates != null) null else return
            }
    ...
    private fun observeEffects() = observeReads { updateEffects() }
